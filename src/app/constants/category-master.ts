export const ADMIN_CATEGORY_MASTER = [
  {
    category: "womens",
    subCategories: [
      // 👗 Clothing - Western
      { key: "tops", label: "Tops & Blouses" },
      { key: "tshirts", label: "T-Shirts" },
      { key: "shirts", label: "Shirts" },
      { key: "crop_tops", label: "Crop Tops" },
      { key: "tunics", label: "Tunics" },
      { key: "dresses", label: "Dresses" },
      { key: "gowns", label: "Gowns" },
      { key: "jumpsuits", label: "Jumpsuits & Playsuits" },

      // 👖 Bottom Wear
      { key: "jeans", label: "Jeans" },
      { key: "trousers", label: "Trousers & Formal Pants" },
      { key: "leggings", label: "Leggings & Jeggings" },
      { key: "palazzos", label: "Palazzos" },
      { key: "skirts", label: "Skirts" },
      { key: "shorts", label: "Shorts" },
      { key: "trackpants", label: "Track Pants & Joggers" },

      // 🪔 Ethnic Wear
      { key: "sarees", label: "Sarees" },
      { key: "kurtas", label: "Kurtas & Kurtis" },
      { key: "salwar_suits", label: "Salwar Suits" },
      { key: "lehenga_choli", label: "Lehenga Cholis" },
      { key: "ethnic_sets", label: "Ethnic Sets" },
      { key: "dupattas", label: "Dupattas & Shawls" },

      // 🧥 Winter & Seasonal
      { key: "jackets", label: "Jackets & Blazers" },
      { key: "sweaters", label: "Sweaters & Sweatshirts" },
      { key: "hoodies", label: "Hoodies" },
      { key: "winterwear", label: "Winterwear" },
      { key: "rainwear", label: "Rainwear" },

      // 🛌 Innerwear & Nightwear
      { key: "bras", label: "Bras" },
      { key: "panties", label: "Panties" },
      { key: "shapewear", label: "Shapewear" },
      { key: "nightwear", label: "Nightwear" },
      { key: "thermals", label: "Thermals" },

      // 👠 Footwear
      { key: "heels", label: "Heels" },
      { key: "flats", label: "Flats" },
      { key: "sandals", label: "Sandals" },
      { key: "slippers", label: "Slippers & Flip Flops" },
      { key: "sports_shoes", label: "Sports Shoes" },
      { key: "casual_shoes", label: "Casual Shoes" },
      { key: "boots", label: "Boots" },
      { key: "ethnic_footwear", label: "Ethnic Footwear" },

      // 👜 Bags
      { key: "handbags", label: "Handbags" },
      { key: "clutches", label: "Clutches" },
      { key: "wallets", label: "Wallets" },
      { key: "backpacks", label: "Backpacks" },
      { key: "tote_bags", label: "Tote Bags" },
      { key: "sling_bags", label: "Sling Bags" },

      // 💍 Accessories
      { key: "jewellery", label: "Jewellery" },
      { key: "watches", label: "Watches" },
      { key: "belts", label: "Belts" },
      { key: "scarves", label: "Scarves & Stoles" },
      { key: "sunglasses", label: "Sunglasses" },
      { key: "hair_accessories", label: "Hair Accessories" },

      // 💄 Beauty & Cosmetics
      { key: "lipsticks", label: "Lipsticks" },
      { key: "foundation", label: "Foundation" },
      { key: "compact_powder", label: "Compact Powder" },
      { key: "kajal_eyeliner", label: "Kajal & Eyeliner" },
      { key: "mascara", label: "Mascara" },
      { key: "nail_polish", label: "Nail Polish" },
      { key: "makeup_kits", label: "Makeup Kits" },

      // 🧴 Personal Care
      { key: "skincare", label: "Skincare" },
      { key: "haircare", label: "Haircare" },
      { key: "bath_body", label: "Bath & Body" },
      { key: "fragrances", label: "Perfumes & Deodorants" },
      { key: "sanitary_products", label: "Sanitary Products" },

      // 🏋️ Activewear
      { key: "gym_wear", label: "Gym Wear" },
      { key: "yoga_wear", label: "Yoga Wear" },
      { key: "sports_bras", label: "Sports Bras" },

      // 🤰 Special Categories
      { key: "maternity_wear", label: "Maternity Wear" },
      { key: "plus_size", label: "Plus Size Clothing" }

    ]
  },
  {
    category: "mens",
    subCategories: [
      // 👕 Top Wear
      { key: "shirts", label: "Shirts" },
      { key: "tshirts", label: "T-Shirts" },
      { key: "polos", label: "Polo T-Shirts" },
      { key: "kurtas", label: "Kurtas" },
      { key: "jackets", label: "Jackets" },
      { key: "blazers", label: "Blazers" },
      { key: "sweaters", label: "Sweaters" },
      { key: "hoodies", label: "Hoodies" },
      { key: "sweatshirts", label: "Sweatshirts" },

      // 👖 Bottom Wear
      { key: "jeans", label: "Jeans" },
      { key: "trousers", label: "Trousers & Formal Pants" },
      { key: "chinos", label: "Chinos" },
      { key: "trackpants", label: "Track Pants & Joggers" },
      { key: "shorts", label: "Shorts" },
      { key: "cargos", label: "Cargo Pants" },
      { key: "dhotis", label: "Dhotis" },
      { key: "lungis", label: "Lungis" },

      // 👔 Ethnic Wear
      { key: "sherwanis", label: "Sherwanis" },
      { key: "kurta_sets", label: "Kurta Sets" },
      { key: "nehru_jackets", label: "Nehru Jackets" },
      { key: "ethnic_jackets", label: "Ethnic Jackets" },

      // 🧥 Winter & Seasonal
      { key: "winterwear", label: "Winterwear" },
      { key: "rainwear", label: "Rainwear" },

      // 🛌 Innerwear & Nightwear
      { key: "innerwear", label: "Innerwear" },
      { key: "vests", label: "Vests" },
      { key: "boxers", label: "Boxers" },
      { key: "nightwear", label: "Nightwear" },
      { key: "thermals", label: "Thermals" },

      // 👟 Footwear
      { key: "sports_shoes", label: "Sports Shoes" },
      { key: "casual_shoes", label: "Casual Shoes" },
      { key: "formal_shoes", label: "Formal Shoes" },
      { key: "sandals", label: "Sandals" },
      { key: "slippers", label: "Slippers & Flip Flops" },
      { key: "loafers", label: "Loafers" },
      { key: "boots", label: "Boots" },
      { key: "ethnic_footwear", label: "Ethnic Footwear" },

      // 👜 Bags
      { key: "backpacks", label: "Backpacks" },
      { key: "laptop_bags", label: "Laptop Bags" },
      { key: "gym_bags", label: "Gym Bags" },
      { key: "wallets", label: "Wallets" },

      // ⌚ Accessories
      { key: "watches", label: "Watches" },
      { key: "belts", label: "Belts" },
      { key: "sunglasses", label: "Sunglasses" },
      { key: "caps", label: "Caps" },
      { key: "ties", label: "Ties" },
      { key: "cufflinks", label: "Cufflinks" },
      { key: "bracelets", label: "Bracelets" },
      { key: "chains", label: "Chains" },

      // 💄 Grooming & Personal Care
      { key: "skincare", label: "Skincare" },
      { key: "haircare", label: "Haircare" },
      { key: "beard_care", label: "Beard Care" },
      { key: "perfumes", label: "Perfumes" },
      { key: "deodorants", label: "Deodorants" },
      { key: "trimmers", label: "Trimmers & Grooming Kits" },

      // 🏋️ Activewear
      { key: "gym_wear", label: "Gym Wear" },
      { key: "sports_tshirts", label: "Sports T-Shirts" },
      { key: "sports_shorts", label: "Sports Shorts" },
      { key: "compression_wear", label: "Compression Wear" },

      // 👔 Special
      { key: "plus_size", label: "Plus Size Clothing" }

    ]
  },
  {
    category: "boys",
    subCategories: [

      // 👕 Top Wear
      { key: "tshirts", label: "T-Shirts" },
      { key: "shirts", label: "Shirts" },
      { key: "polos", label: "Polo T-Shirts" },
      { key: "sweatshirts", label: "Sweatshirts" },
      { key: "hoodies", label: "Hoodies" },
      { key: "jackets", label: "Jackets" },
      { key: "sweaters", label: "Sweaters" },

      // 👖 Bottom Wear
      { key: "jeans", label: "Jeans" },
      { key: "trousers", label: "Trousers & Pants" },
      { key: "trackpants", label: "Track Pants & Joggers" },
      { key: "shorts", label: "Shorts" },
      { key: "cargos", label: "Cargo Pants" },

      // 👔 Ethnic Wear
      { key: "kurta_sets", label: "Kurta Sets" },
      { key: "kurtas", label: "Kurtas" },
      { key: "ethnic_jackets", label: "Ethnic Jackets" },
      { key: "sherwanis", label: "Sherwanis" },

      // 🧥 Winter & Seasonal
      { key: "winterwear", label: "Winterwear" },
      { key: "rainwear", label: "Rainwear" },

      // 🛌 Innerwear & Nightwear
      { key: "innerwear", label: "Innerwear" },
      { key: "vests", label: "Vests" },
      { key: "boxers", label: "Boxers" },
      { key: "nightwear", label: "Nightwear" },
      { key: "thermals", label: "Thermals" },

      // 👟 Footwear
      { key: "sports_shoes", label: "Sports Shoes" },
      { key: "casual_shoes", label: "Casual Shoes" },
      { key: "formal_shoes", label: "Formal Shoes" },
      { key: "sandals", label: "Sandals" },
      { key: "slippers", label: "Slippers & Flip Flops" },
      { key: "boots", label: "Boots" },
      { key: "ethnic_footwear", label: "Ethnic Footwear" },

      // 🎒 Bags
      { key: "school_bags", label: "School Bags" },
      { key: "backpacks", label: "Backpacks" },
      { key: "lunch_bags", label: "Lunch Bags" },

      // ⌚ Accessories
      { key: "watches", label: "Watches" },
      { key: "belts", label: "Belts" },
      { key: "caps", label: "Caps" },
      { key: "sunglasses", label: "Sunglasses" },

      // 🧴 Grooming & Personal Care
      { key: "skincare", label: "Skincare" },
      { key: "haircare", label: "Haircare" },
      { key: "perfumes", label: "Perfumes" },
      { key: "deodorants", label: "Deodorants" },

      // 🏃 Activewear
      { key: "sports_tshirts", label: "Sports T-Shirts" },
      { key: "sports_shorts", label: "Sports Shorts" },
      { key: "sports_sets", label: "Sports Sets" }

    ]
  },
  {
    category: "girls",
    subCategories: [

      // 👗 Top Wear
      { key: "tops", label: "Tops & Blouses" },
      { key: "tshirts", label: "T-Shirts" },
      { key: "shirts", label: "Shirts" },
      { key: "crop_tops", label: "Crop Tops" },
      { key: "tunics", label: "Tunics" },
      { key: "sweatshirts", label: "Sweatshirts" },
      { key: "hoodies", label: "Hoodies" },
      { key: "jackets", label: "Jackets" },
      { key: "sweaters", label: "Sweaters" },

      // 👖 Bottom Wear
      { key: "jeans", label: "Jeans" },
      { key: "trousers", label: "Trousers & Pants" },
      { key: "leggings", label: "Leggings" },
      { key: "palazzos", label: "Palazzos" },
      { key: "skirts", label: "Skirts" },
      { key: "shorts", label: "Shorts" },
      { key: "trackpants", label: "Track Pants & Joggers" },

      // 👗 Dresses
      { key: "dresses", label: "Dresses" },
      { key: "gowns", label: "Gowns" },
      { key: "jumpsuits", label: "Jumpsuits & Playsuits" },

      // 🪔 Ethnic Wear
      { key: "kurta_sets", label: "Kurta Sets" },
      { key: "kurtis", label: "Kurtis" },
      { key: "lehenga_choli", label: "Lehenga Cholis" },
      { key: "salwar_suits", label: "Salwar Suits" },
      { key: "ethnic_gowns", label: "Ethnic Gowns" },
      { key: "dupattas", label: "Dupattas" },

      // 🧥 Winter & Seasonal
      { key: "winterwear", label: "Winterwear" },
      { key: "rainwear", label: "Rainwear" },

      // 🛌 Innerwear & Nightwear
      { key: "innerwear", label: "Innerwear" },
      { key: "nightwear", label: "Nightwear" },
      { key: "thermals", label: "Thermals" },

      // 👟 Footwear
      { key: "flats", label: "Flats" },
      { key: "sandals", label: "Sandals" },
      { key: "sports_shoes", label: "Sports Shoes" },
      { key: "casual_shoes", label: "Casual Shoes" },
      { key: "boots", label: "Boots" },
      { key: "ethnic_footwear", label: "Ethnic Footwear" },

      // 🎒 Bags
      { key: "school_bags", label: "School Bags" },
      { key: "backpacks", label: "Backpacks" },
      { key: "sling_bags", label: "Sling Bags" },

      // 💍 Accessories
      { key: "jewellery", label: "Jewellery" },
      { key: "hair_accessories", label: "Hair Accessories" },
      { key: "watches", label: "Watches" },
      { key: "belts", label: "Belts" },
      { key: "sunglasses", label: "Sunglasses" },

      // 💄 Beauty (Teen Category Optional)
      { key: "lip_balms", label: "Lip Balms" },
      { key: "nail_polish", label: "Nail Polish" },
      { key: "skincare", label: "Skincare" },

      // 🏃 Activewear
      { key: "sports_tshirts", label: "Sports T-Shirts" },
      { key: "sports_shorts", label: "Sports Shorts" },
      { key: "sports_sets", label: "Sports Sets" }

    ]
  },

  {
    category: "kids",
    gender: ["boys", "girls"],
    ageGroup: "0-2_years",
    subCategories: [

      // 👶 Everyday Wear
      { key: "onesies", label: "Onesies" },
      { key: "rompers", label: "Rompers" },
      { key: "jumpsuits", label: "Jumpsuits" },
      { key: "bodysuits", label: "Bodysuits" },
      { key: "co_ords", label: "Co-Ord Sets" },

      // 👕 Clothing
      { key: "tshirts", label: "T-Shirts" },
      { key: "shirts", label: "Shirts" },
      { key: "frocks", label: "Frocks" },
      { key: "dresses", label: "Dresses" },
      { key: "tops", label: "Tops" },

      // 👖 Bottom Wear
      { key: "pants", label: "Pants" },
      { key: "leggings", label: "Leggings" },
      { key: "shorts", label: "Shorts" },

      // 💤 Sleepwear
      { key: "nightwear", label: "Night Suits" },
      { key: "sleeping_bags", label: "Sleeping Bags" },

      // 🧥 Winter Wear
      { key: "sweaters", label: "Sweaters" },
      { key: "jackets", label: "Jackets" },
      { key: "hoodies", label: "Hoodies" },
      { key: "winterwear", label: "Winterwear" },
      { key: "thermals", label: "Thermals" },

      // 🩲 Essentials
      { key: "innerwear", label: "Innerwear" },
      { key: "bibs", label: "Bibs" },
      { key: "cloth_nappies", label: "Cloth Nappies" },

      // 👟 Footwear
      { key: "shoes", label: "Shoes" },
      { key: "sandals", label: "Sandals" },
      { key: "slippers", label: "Slippers" },
      { key: "booties", label: "Baby Booties" },
      { key: "prewalker_shoes", label: "Pre-Walker Shoes" },
      { key: "socks", label: "Socks" },
      // 🎀 Accessories
      { key: "caps", label: "Caps & Hats" },
      { key: "mittens", label: "Mittens" },
      { key: "baby_blankets", label: "Baby Blankets" },
      { key: "swaddles", label: "Swaddles" },

      // 🧴 Baby Care
      { key: "skincare", label: "Baby Skincare" },
      { key: "haircare", label: "Baby Haircare" },
      { key: "baby_towels", label: "Baby Towels" }

    ]
  },
  {
    category: "electricals",
    subCategories: [
      // 💡 Lighting
      { key: "bulbs", label: "Bulbs" },
      { key: "tube_lights", label: "Tube Lights" },
      { key: "ceiling_lights", label: "Ceiling Lights" },
      { key: "chandeliers", label: "Chandeliers" },
      { key: "emergency_lights", label: "Emergency Lights" },

      // 🔌 Switches & Sockets
      { key: "switches", label: "Switches" },
      { key: "sockets", label: "Sockets" },
      { key: "extension_boards", label: "Extension Boards" },

      // ⚡ Wires & Cables
      { key: "electrical_wires", label: "Electrical Wires" },
      { key: "cables", label: "Cables" },
      { key: "ethernet_cables", label: "Ethernet Cables" },

      // 🔋 Power & Backup
      { key: "batteries", label: "Batteries" },
      { key: "inverters", label: "Inverters" },
      { key: "ups", label: "UPS" },
      { key: "stabilizers", label: "Stabilizers" },

      // 🌀 Fans & Ventilation
      { key: "ceiling_fans", label: "Ceiling Fans" },
      { key: "table_fans", label: "Table Fans" },
      { key: "pedestal_fans", label: "Pedestal Fans" },
      { key: "exhaust_fans", label: "Exhaust Fans" },

      // 🔥 Heating & Cooling
      { key: "room_heaters", label: "Room Heaters" },
      { key: "geysers", label: "Geysers & Water Heaters" },
      { key: "coolers", label: "Air Coolers" },

      // 🏠 Home Essentials
      { key: "irons", label: "Irons" },
      { key: "mixers", label: "Mixers & Grinders" },
      { key: "toasters", label: "Toasters" },
      { key: "kettles", label: "Electric Kettles" },

      // 🔧 Tools & Accessories
      { key: "drill_machines", label: "Drill Machines" },
      { key: "screwdrivers", label: "Screwdrivers" },
      { key: "testers", label: "Electrical Testers" }
    ]
  },
  {
    category: "electronics",
    subCategories: [
      // 📱 Mobile Devices
      { key: "smartphones", label: "Smartphones" },
      { key: "feature_phones", label: "Feature Phones" },
      { key: "tablets", label: "Tablets & iPads" },

      // 💻 Computers & Accessories
      { key: "laptops", label: "Laptops" },
      { key: "desktops", label: "Desktops & All-in-One PCs" },
      { key: "monitors", label: "Monitors" },
      { key: "keyboards", label: "Keyboards" },
      { key: "mice", label: "Computer Mouse" },
      { key: "printers", label: "Printers & Scanners" },
      { key: "storage", label: "Hard Drives, SSDs & Storage" },
      { key: "webcams", label: "Webcams" },
      { key: "laptop_bags", label: "Laptop Bags" },

      // 🎧 Audio & Music
      { key: "headphones", label: "Headphones & Earphones" },
      { key: "earbuds", label: "Wireless Earbuds" },
      { key: "bluetooth_speakers", label: "Bluetooth Speakers" },
      { key: "soundbars", label: "Soundbars & Home Audio" },

      // 🔌 Charging & Accessories
      { key: "chargers_cables", label: "Chargers & Cables" },
      { key: "power_banks", label: "Power Banks" },
      { key: "wireless_chargers", label: "Wireless Chargers" },
      { key: "mobile_cases", label: "Mobile Cases & Covers" },
      { key: "screen_protectors", label: "Screen Protectors" },
      { key: "memory_cards", label: "Memory Cards & Pen Drives" },
      { key: "pendrives", label: "Pen Drives" },
      { key: "hubs_docks", label: "USB Hubs & Docks" },

      // 📷 Cameras
      { key: "cameras", label: "DSLR & Mirrorless Cameras" },
      { key: "action_cameras", label: "Action Cameras" },
      { key: "camera_accessories", label: "Camera Lenses, Tripods & Accessories" },

      // 📺 Entertainment
      { key: "televisions", label: "Televisions" },
      { key: "tv_accessories", label: "TV Accessories" },
      { key: "projectors", label: "Projectors" },

      // 🎮 Gaming
      { key: "gaming_consoles", label: "Gaming Consoles & Controllers" },
      { key: "gaming_controllers", label: "Gaming Controllers" },
      { key: "gaming_headsets", label: "Gaming Headsets" },

      // ⌚ Wearables
      { key: "smartwatches", label: "Smartwatches" },
      { key: "fitness_bands", label: "Fitness Bands" },
      { key: "vr_headsets", label: "VR & AR Devices" },

      // 🏠 Smart Home
      { key: "smart_home", label: "Smart Home Devices" },
      { key: "security_cameras", label: "CCTV & Security Cameras" },

      // 🚁 Others
      { key: "external_hard_drives", label: "External Hard Drives" },
      { key: "drones", label: "Drones" }
    ]
  },
  {
    category: "home_kitchen",
    subCategories: [

      // 🍳 Kitchen Essentials
      { key: "cookware", label: "Cookware" },
      { key: "pressure_cookers", label: "Pressure Cookers" },
      { key: "fry_pans", label: "Fry Pans & Tawas" },
      { key: "kitchen_tools", label: "Kitchen Tools & Gadgets" },
      { key: "knives", label: "Knives & Choppers" },
      { key: "storage_containers", label: "Storage Containers" },
      { key: "water_bottles", label: "Water Bottles" },
      { key: "flasks", label: "Flasks & Thermos" },
      { key: "lunch_boxes", label: "Lunch Boxes" },
      { key: "dinnerware", label: "Dinner Sets & Plates" },
      { key: "glassware", label: "Glasses & Cups" },

      // 🍽️ Dining
      { key: "bowls", label: "Bowls" },
      { key: "cutlery", label: "Cutlery" },
      { key: "serving_items", label: "Serving Bowls & Trays" },

      // 🏡 Home Essentials
      { key: "bedsheets", label: "Bedsheets" },
      { key: "blankets", label: "Blankets & Quilts" },
      { key: "pillows", label: "Pillows & Cushions" },
      { key: "curtains", label: "Curtains" },
      { key: "door_mats", label: "Door Mats" },
      { key: "bathroom_accessories", label: "Bathroom Accessories" },

      // 🧹 Cleaning
      { key: "cleaning_supplies", label: "Cleaning Supplies" },
      { key: "brooms_mops", label: "Brooms & Mops" },
      { key: "dustbins", label: "Dustbins" },

      // 💡 Home Decor
      { key: "wall_decor", label: "Wall Decor" },
      { key: "photo_frames", label: "Photo Frames" },
      { key: "artificial_plants", label: "Artificial Plants" },
      { key: "candles", label: "Candles & Holders" },

      // ⚡ Small Appliances
      { key: "mixers", label: "Mixer Grinders" },
      { key: "electric_kettles", label: "Electric Kettles" },
      { key: "induction_cooktops", label: "Induction Cooktops" },
      { key: "rice_cookers", label: "Rice Cookers" }
    ]
  },
  {
    category: "beauty_personal_care",
    gender: ["men", "women"],
    subCategories: [

      // 🧴 Skin Care
      { key: "face_wash", label: "Face Wash" },
      { key: "face_cream", label: "Face Cream" },
      { key: "moisturizers", label: "Moisturizers" },
      { key: "serums", label: "Face Serums" },
      { key: "sunscreen", label: "Sunscreen" },
      { key: "face_masks", label: "Face Masks" },

      // 💇 Hair Care
      { key: "shampoo", label: "Shampoo" },
      { key: "conditioner", label: "Conditioner" },
      { key: "hair_oil", label: "Hair Oil" },
      { key: "hair_serum", label: "Hair Serum" },
      { key: "hair_color", label: "Hair Color" },

      // 💋 Makeup
      { key: "foundation", label: "Foundation" },
      { key: "compact_powder", label: "Compact Powder" },
      { key: "lipstick", label: "Lipstick" },
      { key: "kajal", label: "Kajal & Eyeliner" },
      { key: "mascara", label: "Mascara" },
      { key: "nail_polish", label: "Nail Polish" },

      // 🛁 Bath & Body
      { key: "soaps", label: "Soaps" },
      { key: "body_wash", label: "Body Wash" },
      { key: "body_lotion", label: "Body Lotion" },
      { key: "hand_wash", label: "Hand Wash" },
      { key: "deodorants", label: "Deodorants" },
      { key: "perfumes", label: "Perfumes" },

      // 🪥 Oral Care
      { key: "toothpaste", label: "Toothpaste" },
      { key: "toothbrush", label: "Toothbrushes" },
      { key: "mouthwash", label: "Mouthwash" },

      // 🪒 Grooming
      { key: "trimmers", label: "Trimmers" },
      { key: "razors", label: "Razors & Blades" },
      { key: "shaving_cream", label: "Shaving Cream" },

      // 👶 Baby Care
      { key: "baby_shampoo", label: "Baby Shampoo" },
      { key: "baby_lotion", label: "Baby Lotion" },
      { key: "baby_powder", label: "Baby Powder" },

      // 🩺 Personal Hygiene
      { key: "sanitary_pads", label: "Sanitary Pads" },
      { key: "wet_wipes", label: "Wet Wipes" },
      { key: "cotton_buds", label: "Cotton Buds" }
    ]
  }


];
